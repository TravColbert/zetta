...
import { getAiConfig, isChatEnabled } from "./ai.js";
...
// Shared by the thinking and the reply text, so it is not a reply-length
// figure: 4096 leaves the model room to decide which tool answers the question
// and still write a few paragraphs. 2048 was set when the model did not think,
// and splitting it two ways risks a reply that stops mid-sentence with a
// stop_reason of max_tokens.
export const MAX_TOKENS = 4096;

// Sonnet 5 is the default. What this assistant has to get right is picking the
// tool that covers a question — checking the topic policy before answering
// anything off-topic, searching and then reading when no curated tool fits —
// and that decision is made in the thinking Sonnet does and Haiku has none of.
//
// Thinking is left unconfigured because it is on by default here. Effort is set
// instead: the default is `high`, which is more than a blog lookup needs, and
// `low` buys latency at the cost of the tool use this prompt depends on.
//
// The effort parameter is rejected on Haiku 4.5, so ANTHROPIC_MODEL can point
// at another Sonnet or an Opus, but not at Haiku.
...
export async function callClaude(messages) {
  ...
  const response = await fetch(API_URL, {
    ...
    body: JSON.stringify({
      ...
      system: getAiConfig().systemPrompt,
...
export async function runToolLoop(messages) {
  ...
  for (let round = 1; round <= MAX_TOOL_ROUNDS; round++) {
    ...
    if (response.stop_reason === "refusal") {
      return {
        reply:
          "Sorry — I can’t help with that one. Ask me about this site instead.",
        trace,
        stopReason: "refusal",
      };
    }
...
export async function handleChat(req, ip) {
  if (!isChatEnabled()) {
    return json(503, { error: "The assistant is not configured on this site." });
  }
